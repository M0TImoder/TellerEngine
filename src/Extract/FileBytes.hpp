...
public:
    ...
    Expected<std::uint64_t, Base::Error>
    SizeOf(std::string_view name) const override {
        ...
        if (failure) {
            return Unexpected<Base::Error>(Base::Error{Base::ErrorCode::NotFound, path.string()});
        }
        return static_cast<std::uint64_t>(size);
    }

    Expected<ByteBuffer, Base::Error> Read(std::string_view name,
                                     std::uint64_t offset,
